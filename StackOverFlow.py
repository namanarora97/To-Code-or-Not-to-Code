...
st.set_page_config(
    page_title="To Code or Not to Code",
    page_icon=":shark",
    layout="wide",
...
#######################################################################################################################################
    ...
    qualListDict = {
        ...
        "UndergradMajor": "Undergrad Major",
        "IDE": "IDE",
        "FormalEducation": "Formal Education",
        "DevType": "Developer Type",
        "HopeFiveYears1": "Near-term Plans",
        "PlatformWorkedWith": "Platforms",
        "FrameworkWorkedWith": "Frameworks",
        "OperatingSystem": "Operating System",
        "VersionControl": "Version Control Utility",
        "CompanySizeQuantSmall": "Company Size",
    }
    ...
    quants[0] = quantListDictReverse.get(
        st.selectbox("Select primary measurement", quantListNice)
    )
    quants[1] = quantListDictReverse.get(
        st.selectbox("Select secondary measurement", reversed(quantListNice))
    )
    # st.write(qual)
    # st.write(quants)
    #
    ...
    title = (
        qualListDict.get(qual)
        + " measured by "
        + quantListDict.get(quants[0])
        + " and "
        + quantListDict.get(quants[1])
    )
    ...
    for i in range(2):
        ...
        row |= base.encode(
            ...
            x=alt.X(
                desiredColumns0[i] + ":Q",
                title=qualListDict.get(desiredColumns0[i][: -6 - i]),
            ),
        ...
        for y_encoding in desiredColumns[i]:
            row |= base.encode(
                ...
                x=alt.X(y_encoding + ":Q", title=quantListDict.get(y_encoding[:-7])),
    ...
    desiredColumns = [
        ["FormalEducation", "RaceEthnicity2", "CareerSatisfaction", "Student", "Age1"],
        [
            "EducationParents1",
            "UndergradMajor",
            "Employment",
    ...
    fullOrder2 = [order for order in gender + sexOrient]

    fullOrder = [
        order
        for order in yearsCoding
        + age
        + companySize
        + edu
        + satisfaction
    ...
    columnDict = {
        ...
        "FormalEducation": "Formal Education",
        "EducationParents1": "Parents' Education",
        "UndergradMajor": "Undergrad Major",
        "Employment": "Employment Status",
        "CareerSatisfaction": "Career Satisfaction",
        "Student": "Student Status",
        "Age1": "Age",
        "YearsCoding1": "Years of Coding",
        "CompanySize1": "Company Size",
        "RaceEthnicity2": "Race/Ethnicity",
    }
    ...
    for i in range(2):
        ...
        row |= base2.encode(
            y=alt.Y(
                desiredColumns0[i],
                title=columnDict.get(desiredColumns0[i]),
                sort=fullOrder2,
                axis=alt.Axis(labels=labels, tickSize=0),
            ),
            color=alt.condition(
                "datum." + desiredColumns0[i] + "!=null",
                alt.Color(
        ...
        for y_encoding in desiredColumns[i]:
            row |= base2.encode(
                y=alt.Y(
                    y_encoding,
                    title=columnDict.get(y_encoding),
                    sort=fullOrder,
                    axis=alt.Axis(labels=labels, tickSize=0),
                ),
                color=alt.condition(
                    "datum." + y_encoding + "!=null",
                    alt.Color(
